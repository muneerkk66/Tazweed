...
const populate = async () => {
  try {
    ...
    const muneerSeller = await createSellerUser(
      'muneer',
      'muneerkk66@gmail.com',
      '87654321',
      'car services',
      timeslots
    )
    ...
    // for each buyer create appointment with muneer seller
    const status = ['pending', 'rejected', 'accepted']

    await Promise.all(
      _.map(buyers, async (buyer, index) => {
        const state = status[Math.floor(Math.random() * 3)]
        const timeslot = muneerSeller.timeslots[index].slots[index]

        console.log('Appointment', buyer.name, muneerSeller.name, timeslot)
        await createAppoint(buyer, muneerSeller, state, timeslot)
      })
    )
